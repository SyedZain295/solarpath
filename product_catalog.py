...
def check_component_compatibility(
    ...
    backup_ok = not backup_needed or (
        inverter.get("backup_capable", False) and (not battery or battery.get("backup_capable", False))
    )
    ...
    checks = [
        {
            "id": "dc_input",
            "passed": dc_ok,
            "label_key": "compat.dc_input",
            "detail": f"{inverter.get('dc_max_kw')} kW DC max ≥ {actual_kwp} kWp ({round(actual_kwp / max(inverter.get('dc_max_kw', 1), 0.1) * 100)}% load)",
        },
        {
            "id": "ac_output",
            "passed": ac_ok,
            "label_key": "compat.ac_output",
            "detail": f"{inverter.get('ac_power_kw')} kW AC ≥ {round(actual_kwp * ac_min_ratio, 1)} kWp needed (ratio {round(actual_kwp / max(inverter.get('ac_power_kw', 1), 0.1), 2)})",
        },
    ]
    if battery:
        checks.extend(
            [
                {
                    "id": "hybrid",
                    "passed": hybrid_ok,
                    "label_key": "compat.hybrid",
                    "detail": "Hybrid inverter required when battery selected",
                },
                {
                    "id": "battery_match",
                    "passed": batt_ok,
                    "label_key": "compat.battery_match",
                    "detail": f"Battery on compatible list ({len(inverter.get('compatible_battery_ids') or [])} approved)",
                },
            ]
        )
    if backup_needed:
        checks.append(
            {
                "id": "backup",
                "passed": backup_ok,
                "label_key": "compat.backup",
                "detail": "Backup / island mode for outage goals",
            }
        )
    ...
    return {
        ...
        "panel": {
            ...
            "model": panel["model"],
            "power_wp": panel["power_wp"],
            "efficiency_pct": panel.get("efficiency_pct"),
        },
        "inverter": {
            "id": inverter["id"],
            "brand": inverter["brand"],
            ...
            "ac_power_kw": inverter["ac_power_kw"],
            "dc_max_kw": inverter.get("dc_max_kw"),
            "mppt_count": inverter.get("mppt_count"),
        },
        "battery": {
            "id": battery["id"],
            "brand": battery["brand"],
