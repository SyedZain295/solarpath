...
from financial_model import build_financial_model, DISCLAIMER
...
from confidence_score import calculate_confidence_score, PVGIS_LIMITATION
...
def calculate_suitability_score(
    ...
    if roof_factor >= 0.95:
        score += 15
        factors.append({"factor": "Roof orientation", "impact": "positive", "detail": "Good roof type for solar generation"})
    elif roof_factor >= 0.85:
        score += 8
        factors.append({"factor": "Roof orientation", "impact": "neutral", "detail": "Acceptable roof type with minor production reduction"})
    else:
        score -= 10
        factors.append({"factor": "Roof orientation", "impact": "negative", "detail": "Suboptimal roof orientation reduces output significantly"})

    if specific_yield >= 1000:
        score += 20
        factors.append({"factor": "Solar irradiance", "impact": "positive", "detail": f"Excellent solar resource ({specific_yield:.0f} kWh/kWp/year)"})
    elif specific_yield >= 850:
        score += 12
        factors.append({"factor": "Solar irradiance", "impact": "positive", "detail": f"Good solar resource ({specific_yield:.0f} kWh/kWp/year)"})
    else:
        score += 5
        factors.append({"factor": "Solar irradiance", "impact": "neutral", "detail": f"Moderate solar resource ({specific_yield:.0f} kWh/kWp/year)"})

    annual_kwh = estimate_annual_consumption_kwh(inp)
    if annual_kwh >= 3000:
        score += 10
        factors.append({"factor": "Electricity consumption", "impact": "positive", "detail": "Sufficient consumption to justify solar investment"})
    else:
        score += 3
        factors.append({"factor": "Electricity consumption", "impact": "neutral", "detail": "Lower consumption – smaller system may be more appropriate"})

    if inp.budget_eur > 0:
        cost_est = system_kwp * COST_PER_KWP["residential"]["typical"]
        if inp.budget_eur >= cost_est:
            score += 10
            factors.append({"factor": "Budget alignment", "impact": "positive", "detail": "Budget covers estimated system cost"})
        elif inp.budget_eur >= cost_est * 0.7:
            score += 5
            factors.append({"factor": "Budget alignment", "impact": "neutral", "detail": "Budget covers most of the estimated cost – financing may help"})
        else:
            score -= 5
            factors.append({"factor": "Budget alignment", "impact": "negative", "detail": "Budget below estimated cost – consider smaller system or financing"})
...
def build_package(
    ...
    product_spec = build_package_spec(
        package_id, system_kwp, battery_kwh, goals,
        roof_type=inp.roof_type, roof_area_m2=inp.roof_area_m2,
    )
...
def generate_recommendation(inp: CalculatorInput, pvgis_data: Optional[dict] = None) -> dict:
    ...
    system_kwp = recommend_system_size_kwp(
        annual_kwh, adjusted_yield, goals,
    ...
    fin_no_batt = calculate_financials_with_upfront(
        inp, pkg_no_batt["upfront_cost"], pkg_no_batt["system_kwp"],
        round(pkg_no_batt["system_kwp"] * adjusted_yield), 0,
    )
    pkg_with_batt = three_packages["packages"]["best_value"]
    fin_with_batt = (
        calculate_financials_with_upfront(
            inp, pkg_with_batt["upfront_cost"], pkg_with_batt["system_kwp"],
            round(pkg_with_batt["system_kwp"] * adjusted_yield), pkg_with_batt["battery_kwh"],
        )
        if pkg_with_batt["battery_kwh"] > 0 else None
    )
    ...
    meter_timeline = estimate_meter_timeline(inp.monthly_bill_eur, inp.monthly_kwh, inp.electricity_price_ct, financials["annual_savings"])
    ...
    assumptions = [
        ...
        f"Maintenance allowance: 0.8%/year of system cost",
    ...
    price_scenarios = build_price_scenarios(financials["annual_savings"], financials["system_cost_typical"], financials["payback_years"])
    financing_comparison = build_financing_comparison(financials["system_cost_typical"], financials["monthly_savings"], financials["annual_savings"])
    ...
    result = {
        ...
        "system_recommendation": {
            ...
            "battery_label": (
                ...
                if primary_spec.get("battery") else None
            ),
...
def _project_tracker() -> list:
    return [
        ...
        {"step": 6, "title_key": "legal.step6_title", "detail_key": "legal.step6_detail", "status": "pending", "deadline_key": "legal.step6_deadline"},
        {"step": 7, "title_key": "legal.step7_title", "detail_key": "legal.step7_detail", "status": "pending"},
        {"step": 8, "title_key": "legal.step8_title", "detail_key": "legal.step8_detail", "status": "pending", "warning_key": "legal.step8_warning"},
...
def _build_why_recommend(goals, system_kwp, battery_kwh, tech, financials, goal_decisions=None) -> list:
    ...
    if "lower_bill" in goals and "backup" not in goals:
        reasons.append("A battery is NOT automatically recommended for bill savings – compare the three packages below.")
