...
def hp_profile_complete(inp) -> bool:
    ...
    if "space_heating" in goals:
        if (
            float(getattr(inp, "hp_heated_area_m2", 0) or 0) <= 0
            and float(getattr(inp, "hp_annual_heat_kwh", 0) or 0) <= 0
        ):
            return False
...
def build_hp_assessment(inp, electricity_price_ct: float = 32.0) -> dict:
    ...
    if getattr(inp, "hp_daytime_heating", "") == "no":
        notes.append(
            "Heating mostly outside solar hours — battery or tariff optimisation helps more than extra panels alone."
        )
