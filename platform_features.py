...
SUPPLIER_VERIFICATION_ITEMS = [
    {"id": "identity", "label": "Identity verified", "description": "Company registration and contact details checked"},
    {
        ...
        "description": "Public liability and professional indemnity on file",
    },
    {
        "id": "certifications",
        "label": "Certifications checked",
        "description": "Relevant electrical and solar certifications reviewed",
    },
    {
        "id": "installations",
        "label": "Recent installations",
        "description": "Portfolio of completed projects in the last 24 months",
    },
    {"id": "response_time", "label": "Typical response time", "description": "Median first response to quote requests"},
    {
        "id": "quote_completeness",
        "label": "Quote completeness score",
        "description": "How often quotes include all checklist items",
    },
    {
        "id": "ratings",
        "label": "Customer rating history",
        "description": "Verified reviews from completed installations",
    },
]
...
LEAD_TIERS = {
    ...
    "qualified": {
        "label": "Qualified",
        "description": "Roof details, ownership, budget, timing, and contact preference",
    },
    "quote_ready": {
        "label": "Quote-ready",
        "description": "Detailed consumption profile, roof info, and desired system type",
    },
...
HOUSEHOLD_LOADS = [
    ...
    {
        "key": "high_daytime_use",
        "label": "High daytime use",
        "icon": "☀️",
        "load_kwh_add": 0,
        "self_consumption_boost": 0.08,
    },
...
def build_household_energy_profile(inp, annual_kwh: float) -> dict:
    ...
    ev_precise = float(getattr(inp, "ev_annual_km", 0) or 0) > 0 and "ev_charging" in (
        getattr(inp, "goals", None) or []
    )
    ...
    return {
        ...
        "lead_value_note": "Detailed load profile improves installer quotes"
        if len(active) >= 2
        else "Add more load details for better quotes",
    }


def calculate_readiness_score(
    inp, solar_viable: bool, adjusted_yield: float, system_kwp: float, annual_kwh: float, financials: dict
) -> dict:
    score = 50
    reasons = []
    ...
    reasons.append(
        {
            ...
            "impact": "positive" if rs >= 12 else "negative",
    ...
    if inp.shading == "none":
        ...
        score += 2
        reasons.append(
            {"factor": "Shading", "impact": "neutral", "detail": "Partial shading – site survey recommended"}
        )
        ...
        reasons.append(
            {
                ...
                "impact": "positive",
                "detail": f"~{annual_kwh:,.0f} kWh/year – enough load to justify PV",
            }
        )
    else:
        score -= 5
        reasons.append(
            {"factor": "Usage pattern", "impact": "negative", "detail": "Low consumption – economics may be weaker"}
        )
    ...
    if adjusted_yield >= 900:
        ...
        reasons.append(
            {
                "factor": "Location solar resource",
                "impact": "positive",
                "detail": f"~{adjusted_yield:.0f} kWh/kWp/year expected",
            }
        )
        ...
        reasons.append(
            {
                "factor": "Location solar resource",
                "impact": "negative",
                "detail": "Below-average solar resource for the site",
            }
        )
    ...
    if payback < 10:
        score += 10
        reasons.append(
            {"factor": "Budget & payback", "impact": "positive", "detail": f"Estimated payback ~{payback} years"}
        )
    elif payback > 16:
        score -= 6
        reasons.append(
            {
                "factor": "Budget & payback",
                "impact": "negative",
                "detail": f"Long payback (~{payback} years) at current assumptions",
            }
        )
    ...
    reasons.append(
        {
            "factor": "Self-consumption potential",
            "impact": "positive" if sc_pct >= 40 else "neutral",
        ...
        reasons.append(
            {
                "factor": "Backup need",
                "impact": "neutral",
                "detail": "Backup goal requires battery + hybrid inverter – higher cost",
            }
        )
        ...
        reasons.append(
            {
                "factor": "Site data",
                "impact": "negative",
                "detail": "Roof dimensions or photos not provided – survey needed",
            }
        )
    elif inp.has_roof_photos:
        score += 5
        reasons.append(
            {"factor": "Site data", "impact": "positive", "detail": "Roof photos available for installer review"}
        )
...
def build_financing_comparison(upfront: float, monthly_savings: float, annual_savings: float) -> dict:
    from financial_model import DISCLAIMER, calculate_financing
...
def build_why_explanation(
    inp, system_kwp: float, battery_kwh: float, annual_kwh: float, annual_production: float, goals: list
) -> str:
    goal_txt = ", ".join(g.replace("_", " ") for g in goals[:3]) if goals else "lower electricity bill"
    roof = inp.roof_type.replace("_", " ")
    batt = (
        f" A {battery_kwh} kWh battery is included for your goals."
        if battery_kwh > 0
        else " We have not assumed a battery unless your goal requires it."
    )
    return (
        ...
        steps.append(
            {
                "phase": "Now",
                "title": "Rooftop PV",
                "detail": f"Start with ~{system_kwp} kWp as assessed",
                "priority": "high",
            }
        )
        ...
        steps.append(
            {
                "phase": "Next",
                "title": "EV charger",
                "detail": "Add smart wallbox when you switch to an EV",
                "priority": "medium",
            }
        )
        ...
        steps.append(
            {
                "phase": "Next",
                "title": "Heat pump",
                "detail": "Pair with PV for lower heating running costs",
                "priority": "medium",
            }
        )
    steps.append(
        {
            "phase": "Later",
            "title": "Add battery",
            "detail": "Retrofit storage if backup or evening use becomes important",
            "priority": "low",
        }
    )
    steps.append(
        {
            "phase": "Later",
            "title": "Smart tariff",
            "detail": "Dynamic tariff to shift loads to cheap/solar hours",
            "priority": "low",
        }
    )
        ...
        steps.append(
            {
                "phase": "Later",
                "title": "Expand PV",
                "detail": "Additional roof area may allow system expansion",
                "priority": "low",
            }
        )
    return steps
    ...
    return {
        ...
        "sections": [
            ...
            {"id": "co2", "title": "CO₂ estimate", "included": True},
            {
                ...
                "included": bool(recommendation.get("battery_comparison")),
            },
...
def build_quote_status(quote: dict) -> list:
    ...
    try:
        ...
        idx = 0
    return [
        {"id": s["id"], "label": s["label"], "label_key": s.get("label_key", ""), "done": i <= idx, "current": i == idx}
        for i, s in enumerate(QUOTE_STATUS_STEPS)
    ]
