document.addEventListener('DOMContentLoaded', () => {
  ...
  async function resolveLocationFields() {
    ...
    try {
      ...
      if (!resp.ok) return Boolean(plz || loc);
      if (field('latitude') && data.latitude != null) field('latitude').value = data.latitude;
      if (field('longitude') && data.longitude != null) field('longitude').value = data.longitude;
      if (!loc && data.display_name && field('location')) {
        field('location').value = data.display_name;
      }
      return true;
    } catch {
      return Boolean(plz || loc);
    }
  }
