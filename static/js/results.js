...
function getRecommendationRaw() {
  if (window.__DEMO_RECOMMENDATION_RAW__) {
    return window.__DEMO_RECOMMENDATION_RAW__;
  }
  const demoEl = document.getElementById('demo-recommendation-data');
  if (demoEl) {
    const txt =
      (demoEl.textContent && demoEl.textContent.trim()) ||
      (demoEl.innerHTML && demoEl.innerHTML.trim()) ||
      (demoEl.innerText && demoEl.innerText.trim()) ||
      '';
    if (txt) return txt;
  }
  return sessionStorage.getItem('solarRecommendation') || localStorage.getItem('solarRecommendation');
}

async function loadRecommendationRaw() {
  if (window.__DEMO_RECOMMENDATION_RAW__) {
    return window.__DEMO_RECOMMENDATION_RAW__;
  }
  const embedded = getRecommendationRaw();
  if (embedded && document.getElementById('demo-recommendation-data')) {
    return embedded;
  }
  const demoEl = document.getElementById('demo-recommendation-data');
  if (demoEl) {
    try {
      const resp = await fetch('/api/demo-recommendation', { credentials: 'same-origin' });
      if (resp.ok) {
        return JSON.stringify(await resp.json());
      }
    } catch (_) {}
  }
  return embedded || getRecommendationRaw();
}

function bootstrapResultsPage(raw) {
  if (!raw) {
  ...
  try {
    sessionStorage.setItem('solarRecommendation', raw);
  } catch (_) {}
...
window.bootstrapResultsPage = bootstrapResultsPage;

document.addEventListener('DOMContentLoaded', async () => {
  bootstrapResultsPage(await loadRecommendationRaw());
});

function renderResultsHero(d) {
  const pkgs = d.three_packages?.packages || {};
  const pkg = pkgs[selectedPackageId] || pkgs.best_value || pkgs.cheapest;
  const savings = pkg?.annual_savings ?? d.financial_model?.annual_savings;
  const kwp = d.system_recommendation?.headline_kwp ?? pkg?.system_kwp ?? '—';
  const payback = pkg?.payback_years ?? d.financial_model?.payback_years ?? '—';
  const liveData = !d.pvgis_fallback;
  const region = (d.location?.name || '').includes('Bayern') || (d.location?.name || '').includes('Bavaria')
    ? 'Bayern'
    : (window.REGION_FOCUS || 'Bayern');

  return `
    <section class="results-hero" aria-label="${tr('results.hero_aria', 'Key outcomes')}">
      <div class="results-hero-badges">
        <span class="trust-badge trust-badge--region">${region} · PVGIS</span>
        <span class="trust-badge trust-badge--data">${liveData
          ? tr('results.badge_live_yield', 'Live solar yield data')
          : tr('results.badge_regional_yield', 'Regional yield estimate')}</span>
      </div>
      <div class="results-hero-metrics">
        <div class="results-hero-stat results-hero-stat--primary">
          <span class="results-hero-label">${tr('results.annual_savings', 'Annual savings')}</span>
          <strong class="metric-money">${savings != null ? formatCurrency(savings) : '—'}</strong>
          <span class="results-hero-hint">${tr('results.per_year', 'per year vs grid')}</span>
        </div>
        <div class="results-hero-stat">
          <span class="results-hero-label">${tr('results.recommended_size', 'Recommended size')}</span>
          <strong>${kwp} kWp</strong>
          <span class="results-hero-hint">${tr('results.pvgis_sized', 'Sized from your usage')}</span>
        </div>
        <div class="results-hero-stat">
          <span class="results-hero-label">${tr('results.payback', 'Payback')}</span>
          <strong>${payback} ${tr('results.years_short', 'yrs')}</strong>
          <span class="results-hero-hint">${tr('results.estimated_payback', 'Estimated simple payback')}</span>
        </div>
      </div>
    </section>`;
}
  ...
  document.querySelectorAll('.package-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      if (heroEl) {
        ...
        hw.innerHTML = renderResultsHero(d);
        heroEl.replaceWith(hw.firstElementChild);
      }
    });
  });
  ...
  return `
    <div class="package-card package-card--${pkg.id || 'default'} ${selected ? 'selected' : ''} ${pkg.recommended ? 'recommended' : ''}" data-package-id="${pkg.id}">
      ...
      <div class="package-metrics package-metrics--aligned">
        ...
        <div><span>${tr('results.annual_savings', 'Annual savings')}</span><strong class="metric-money">${formatCurrency(pkg.annual_savings)}</strong></div>
