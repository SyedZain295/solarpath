...
function renderHouseholdProfile(hp) {
  ...
  const loadsHtml = hasLoads
    ...
    : `<div class="household-empty-state">
        ...
        <a href="/calculator" class="household-empty-cta">${tr('results.add_in_calculator', 'Add these in the calculator')} →</a>
      </div>`;
  ...
  return `
    <section class="result-section matched-suppliers-section">
      <div class="matched-suppliers-panel">
        ...
        <div class="matched-suppliers-trust-row">
          <span class="trust-badge trust-badge--region">Bayern · ${tr('results.installer_network', 'Installer network')}</span>
          <span class="trust-badge trust-badge--data">${tr('results.fit_scored', 'Fit-scored matches')}</span>
        </div>
