...
def _warnings(q: dict, all_quotes: list[dict]) -> list[dict]:
    ...
    if cpk and cpk < CPK_LOW_WARN:
        warns.append(
            {"level": "high", "code": "cpk_low", "message": f"€/kWp unusually low ({cpk:,} €) — check what's excluded."}
        )
        ...
        warns.append(
            {
                "level": "medium",
                "code": "cpk_high",
                "message": f"€/kWp high ({cpk:,} €) — verify premium components justify cost.",
            }
        )
        ...
        warns.append(
            {
                "level": "high",
                "code": "no_grid",
                "message": "Grid-operator registration not stated — often €500–1,500 extra.",
            }
        )
    if not q["checks"].get("includes_mastr"):
        warns.append(
            {"level": "medium", "code": "no_mastr", "message": "MaStR registration not stated — required in Germany."}
        )
        ...
        warns.append(
            {
                "level": "medium",
                "code": "no_electrical",
                "message": "Electrical / meter cabinet work not clearly included.",
            }
        )
        ...
        warns.append(
            {
                "level": "medium",
                "code": "short_warranty",
                "message": f"Module warranty only {q['warranty_years']} years — typical is 20–25.",
            }
        )
        ...
        warns.append(
            {
                "level": "low",
                "code": "short_validity",
                "message": f"Quote valid only {q['validity_days']} days — limited time to decide.",
            }
        )
        ...
        warns.append(
            {
                ...
                "message": "Inverter may be undersized vs DC capacity — confirm AC/DC ratio.",
            }
        )
        ...
        warns.append(
            {
                "level": "medium",
                "code": "incomplete",
                "message": "Many standard line items not mentioned — ask for itemised quote.",
            }
        )
            ...
            warns.append(
                {
                    ...
                    "message": "Total cost much lower than other quotes — verify scope matches.",
                }
            )
    return warns
...
def compare_quotes(raw_quotes: list[dict]) -> dict[str, Any]:
    quotes = [
        normalize_quote(q)
        for q in raw_quotes
        if q.get("total_eur") or (q.get("installer") and q.get("installer") != "—")
    ]
    if not quotes:
    ...
    rows = [
        {"key": "total_eur", "label": "Total cost", "values": [q["total_eur"] for q in quotes]},
        {
            "key": "cost_per_kwp",
            "label": "€/kWp",
            "values": [q.get("cost_per_kwp") for q in quotes],
            "highlight": "lowest",
        },
        {"key": "kwp", "label": "System size (kWp)", "values": [q.get("kwp") for q in quotes]},
        {
            "key": "panels",
            "label": "Panel model",
            "values": [f"{q['panel_count']}× {q['panels']}" if q.get("panel_count") else q["panels"] for q in quotes],
        },
        {
            "key": "inverter",
            "label": "Inverter",
        ...
        {
            "key": "includes_mounting",
            "label": "Mounting",
            "values": [q["checks"].get("includes_mounting") for q in quotes],
            "bool": True,
        },
        {
            "key": "includes_electrician",
            "label": "Electrical work",
            "values": [q["checks"].get("includes_electrician") for q in quotes],
            "bool": True,
        },
        {
            "key": "includes_grid_registration",
            "label": "Grid registration",
            "values": [q["checks"].get("includes_grid_registration") for q in quotes],
            "bool": True,
        },
        {
            "key": "includes_mastr",
            "label": "MaStR",
            "values": [q["checks"].get("includes_mastr") for q in quotes],
            "bool": True,
        },
        ...
        {"key": "completeness_pct", "label": "Completeness", "values": [q["completeness_pct"] for q in quotes]},
        {
            "key": "value_score",
            "label": "Value score",
            "values": [q["value_score"] for q in quotes],
            "highlight": "highest",
        },
