...
def test_beta_gate_returns_json_for_api():
    ...
    with app.test_request_context("/api/other", method="POST"):
        with (
            patch.object(beta_access, "beta_gate_enabled", return_value=True),
            patch.object(beta_access, "check_beta_access", return_value=False),
        ):
...
def test_beta_gate_disabled_via_env(monkeypatch):
    monkeypatch.setenv("BETA_GATE_ENABLED", "0")
    monkeypatch.setenv("BETA_DEMO_MODE", "1")
    assert beta_access.beta_gate_enabled() is False


def test_production_defaults_public_demo(monkeypatch):
    monkeypatch.delenv("BETA_GATE_ENABLED", raising=False)
    monkeypatch.delenv("BETA_DEMO_MODE", raising=False)
    monkeypatch.setenv("APP_ENV", "production")
    monkeypatch.setenv("FLASK_ENV", "production")
    assert beta_access.beta_gate_enabled() is False
    assert beta_access._beta_demo_mode() is True
...
def test_persist_invite_session():
    ...
    with app.test_request_context("/?invite=tok"):
        with (
            patch.object(beta_access, "_beta_invite_tokens", return_value={"tok"}),
            patch.object(beta_access, "beta_gate_enabled", return_value=True),
        ):
...
def test_check_beta_access_accepts_invite_header():
    ...
    with app.test_request_context("/api/calculate", method="POST", headers={"X-Beta-Invite": "tok"}):
        with (
            patch.object(beta_access, "_beta_invite_tokens", return_value={"tok"}),
            patch.object(beta_access, "_beta_access_password", return_value=""),
        ):
            assert beta_access.check_beta_access() is True
...
def test_demo_route_public(client):
    r = client.get("/demo")
    assert r.status_code == 200
    assert b"demo-banner" in r.data
    assert b'id="demo-recommendation-data"' in r.data
    assert b"__DEMO_RECOMMENDATION_RAW__" in r.data
    assert b"three_packages" in r.data
    assert b"M" in r.data  # München sample embedded in page JSON


def test_demo_recommendation_api(client):
    r = client.get("/api/demo-recommendation")
    assert r.status_code == 200
    data = r.get_json()
    assert data["location"]["name"]
    assert "three_packages" in data
