...
def prepare_supplier_for_public_listing(supplier: dict, *, quality_score: int | None = None) -> dict:
    ...
    row["display_phone"] = (
        None if is_placeholder_phone(supplier.get("phone")) or row["is_demo_listing"] else supplier.get("phone")
    )
    row["display_email"] = (
        None
        if is_placeholder_email(supplier.get("email"), supplier.get("company_name", "")) or row["is_demo_listing"]
        else supplier.get("email")
    )
