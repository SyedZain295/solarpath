...
from flask import Flask, g, jsonify, make_response, redirect, render_template, request, send_file, session, url_for

import supplier_store
from analytics import beta_metrics_summary, track_event
...
from auth_supplier import get_current_supplier_id, login_supplier, supplier_authorized
...
from email_service import notify_quote_request, send_email_with_attachment, smtp_status
...
from ev_marketplace import clear_vehicle_cache, home_energy_check, match_vehicles, vehicle_by_slug, vehicles_for_api
from ev_media_store import media_file_path, save_dealer_cert, save_dealer_photos
from ev_profile import apply_ev_fields_to_input
...
from logging_config import audit, setup_logging
from security_events import auth_blocked, record_auth_failure, record_auth_success
from pdf_report import generate_decision_report_pdf
from platform_features import build_quote_status, match_suppliers
from price_list_import import merge_products, parse_csv_text
from product_catalog import catalog_for_ui, check_component_compatibility, find_alternatives, load_catalog, save_catalog
from pvgis_client import geocode_location, geocode_postcode, get_pv_estimate, roof_type_to_pvgis_params
...
from supplier_utils import (
    ...
    prepare_supplier_for_public_listing,
    public_installer_brand,
)

settings = get_settings()
REGION_FOCUS = settings.region_focus
...
init_db()
register_http_middleware(app, production=IS_PRODUCTION)
...
def admin_authorized() -> bool:
    if session.get("admin_authenticated"):
        return True
    token = (
        request.headers.get("X-Admin-Token") or request.args.get("admin_token") or request.form.get("admin_token") or ""
    )
...
def _coord_or_none(value) -> float | None:
...
def resolve_calculator_location(data: dict) -> tuple[float, float, str, str | None]:
    """Resolve lat/lon for calculator API from coords, PLZ, or location name."""
    lat = _coord_or_none(data.get("latitude"))
    lon = _coord_or_none(data.get("longitude"))
    location_name = (data.get("location_name") or "").strip()
    postcode = (data.get("postcode") or "").strip()
    error: str | None = None

    if lat is not None and lon is not None:
        return lat, lon, location_name or postcode, None

    if postcode:
        geo = postcode_geocode_cached(postcode, city=location_name)
        if geo:
            return (
                float(geo["latitude"]),
                float(geo["longitude"]),
                geo.get("display_name") or location_name or postcode,
                None,
            )
        error = (
            f"Could not resolve postcode {postcode}. "
            "Add a city name or enter latitude/longitude."
        )

    if location_name and (lat is None or lon is None):
        geo = geocode_cached(location_name)
        if geo:
            return (
                float(geo["latitude"]),
                float(geo["longitude"]),
                geo.get("display_name") or location_name,
                None,
            )
        if not error:
            error = "Could not geocode location. Please try coordinates or a different address."

    if not postcode and not location_name:
        error = "Location is required. Enter a postcode (PLZ), city, or coordinates."

    return 0.0, 0.0, location_name, error
...
def search_suppliers_by_location(
    ...
    *,
    ...
    if state:
        ...
        q = region.lower()
        filtered = [s for s in filtered if any(q in r.lower() or r.lower() in q for r in s.get("regions", []))]
...
def supplier_nearest_distance_km(supplier: dict, origin: dict, search_postcode: str = ""):
    ...
    if lat is not None and lon is not None:
        ...
        nearest_postcode = (
            supplier.get("locations_served", [None])[0] if supplier.get("locations_served") else search_plz or "—"
        )
...
def demo():
    ...
    resp = make_response(render_template("demo.html", demo_data=load_demo_recommendation()))
    # Prevent stale cached HTML/JS causing demo bootstrap to miss embedded demo data.
...
@app.route("/api/demo-recommendation")
def demo_recommendation_api():
    """Fixed München sample recommendation for the public demo route."""
    resp = jsonify(load_demo_recommendation())
    resp.headers["Cache-Control"] = "no-store, no-cache, must-revalidate, max-age=0"
    resp.headers["Pragma"] = "no-cache"
    resp.headers["Expires"] = "0"
    return resp
...
def get_translated_plans(lang: str) -> dict:
    ...
    for pid, plan in SUPPLIER_PLANS.items():
        features = [translate(lang, f"plans.{pid}.feature{i}", feat) for i, feat in enumerate(plan.get("features", []))]
        plans[pid] = {
            **plan,
...
def admin_page():
    if request.method == "POST":
        if auth_blocked("admin"):
            return render_template("admin_login.html", error=True), 429
        token = (request.form.get("admin_token") or "").strip()
        if token and secrets.compare_digest(token, ADMIN_TOKEN):
            session["admin_authenticated"] = True
            record_auth_success("admin_login_success")
            return redirect("/admin")
        record_auth_failure("admin_login_failed", identifier="admin")
...
def api_ev_dealer_login():
    ...
    password = (data.get("password") or "").strip()
    if auth_blocked(email):
        return jsonify({"error": "Too many attempts. Try again later."}), 429
    dealer = dealer_by_email(email)
    if not dealer or not verify_password(dealer, password):
        record_auth_failure("ev_dealer_login_failed", identifier=email, email=email)
        return jsonify({"error": "Invalid email or password"}), 401
    if dealer.status == "suspended":
        record_auth_failure("ev_dealer_login_blocked", identifier=email, email=email, reason="suspended")
        return jsonify({"error": "Account suspended"}), 403
    login_ev_dealer(dealer.id)
    record_auth_success("ev_dealer_login_success", dealer_id=dealer.id)
...
def api_ev_buyer_lead():
    ...
    try:
        track_event(
            "ev_buyer_lead", {"vehicle_slug": slug, "qualified": result.get("qualified"), "demo": result.get("demo")}
        )
    except Exception:
        pass
    if not result.get("ok"):
...
def beta_login():
    error = None
    if request.method == "POST":
        if auth_blocked("beta"):
            error = "Too many attempts. Try again later."
            return render_template("beta_login.html", error=error, beta_gate=beta_gate_enabled()), 429
        password = request.form.get("password", "")
        if verify_beta_password(password):
            session["beta_authenticated"] = True
            session.permanent = True
            record_auth_success("beta_login_success")
            nxt = request.args.get("next") or "/"
            return redirect(nxt)
        record_auth_failure("beta_login_failed", identifier="beta")
...
def api_quick_estimate():
    ...
    def _num(key: str, default: float = 0.0) -> float | None:
    ...
    monthly_kwh = _num("monthly_kwh", 0)
    if monthly_kwh is None:
        return jsonify({"error": "invalid monthly_kwh"}), 400
    ...
    if postcode:
        ...
        if geo:
            try:
                ...
                pvgis = get_pv_estimate(
                    float(geo["latitude"]), float(geo["longitude"]), peakpower=1.0, angle=angle, aspect=aspect
                )
    ...
    result = quick_estimate_range(
        ...
        monthly_bill_eur=_num("monthly_bill_eur", 0) or 0,
        electricity_price_ct=_num("electricity_price_ct", 32.0) or 32.0,
        roof_area_m2=_num("roof_area_m2", 0) or 0,
        roof_type=data.get("roof_type", "pitched_south"),
        goals=goals,
...
def api_calculate():
    ...
    lat, lon, location_name, loc_error = resolve_calculator_location(data)
    if loc_error:
        return jsonify({"error": loc_error}), 400
    ...
    recommendation["pvgis"] = (
        {
            k: pvgis.get(k)
            for k in ("specific_yield_kwh_kwp", "annual_kwh_per_kwp", "cached", "monthly")
            if pvgis.get(k) is not None
        }
        if pvgis
        else None
    )
...
def api_geocode():
    ...
    if len(q) == 5 and q.isdigit():
        result = postcode_geocode_cached(q)
    else:
        result = geocode_cached(q)
    if result:
        return jsonify(result)
...
def api_suppliers_list():
    ...
    filtered = [prepare_supplier_for_public_listing(s, quality_score=_supplier_quality_score(s)) for s in filtered]
...
SUPPLIER_WRITABLE_FIELDS = {
    ...
    "earliest_survey_date",
    "earliest_install_weeks",
    "residential_available",
    "commercial_available",
    "battery_capable",
    "ev_charger_capable",
    "heat_pump_capable",
    "agricultural_available",
    "products",
}
...
def api_login():
    ...
    if not customer or not verify_password(customer, password):
        record_auth_failure("customer_login_failed", identifier=email, email=email)
        return jsonify({"error": "Invalid email or password."}), 401

    login_customer(customer.id)
    record_auth_success("customer_login_success", customer_id=customer.id)
...
def api_supplier_product_delete(supplier_id, product_id):
    ...
    if not existing:
        return jsonify({"error": "Supplier not found"}), 404
    products = [
        p for p in (existing.get("products") or []) if p.get("id") != product_id and p.get("name") != product_id
    ]
...
def api_supplier_price_list_upload(supplier_id):
    ...
    return jsonify(
        {"ok": True, "imported": len(imported), "total_products": len(merged), "errors": errors, "supplier": saved}
    )
...
def api_quotes_create():
    ...
    if not all(data.get(f) for f in required):
        return jsonify(
            {"error": "Name, email, phone, and postcode are required", "error_key": "api.error.missing_fields"}
        ), 400
        ...
        matched = [
            {
                **source_installer,
                "fit_score": 100,
                "fit_label": "Branded intake",
                "fit_reasons": ["Direct enquiry via installer form"],
            }
        ]
    ...
    quote = {
        ...
        "matched_suppliers": [
            {"id": s["id"], "company_name": s.get("company_name"), "fit_score": s.get("fit_score")}
            for s in matched
    ...
    return jsonify(
        {
            ...
            "quote_id": quote["id"],
            "quote": quote,
            "message": "Quote request sent to matched suppliers.",
        }
    ), 201
    ...
    if supplier_id:
        ...
        with db_session() as db:
            q = db.query(Quote).filter(
                (Quote.customer_id == customer.id) | (Quote.customer_email == customer.email.lower())
            )
...
def api_list_assessments():
    ...
    with db_session() as db:
        q = db.query(Assessment).filter(
            (Assessment.customer_id == customer.id) | (Assessment.customer_email == customer.email.lower())
        )
