...
USAGE_BREAKDOWN = {
    ...
    "kitchen": {
        "pct": 18,
        "label": "Kitchen (cooking, fridge, dishwasher)",
        "icon": "🍳",
        "tips": [
...
def estimate_meter_timeline(
    monthly_bill: float, monthly_kwh: float, electricity_price_ct: float, annual_savings_with_solar: float = 0
) -> dict:
    ...
    past = [
        {
            "year": f"Year -{i}",
            "annual_cost": round(annual_cost / ((1 + escalation) ** i)),
            "note": "Estimated from current usage",
        }
    ...
    for i in range(1, 11):
        ...
        future_with.append(
            {
                "year": f"Year +{i}",
                "annual_cost": round(cost_with_solar),
                "savings": round(cost_no_solar - cost_with_solar),
            }
        )
