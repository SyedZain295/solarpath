    ...
    return {
        ...
        "location": {
            ...
            "service_area_match": any(
                _supplier_in_service_area(s, contact.get("customer_postcode", "")) for s in matched_suppliers
            ),
        },
        "customer_status": {
...
def evaluate_qualified_lead(
    rec: dict, contact: dict, matched_suppliers: list, selected_supplier_ids: list | None = None
) -> dict:
    ci = rec.get("calculator_inputs") or {}
    ...
    add(
        "consent",
        "lead.rule.consent",
        bool(contact.get("consent_contact")) and bool(contact.get("consent_share_installers")),
    )
    ...
    area_match = bool(pool) and any(
        _supplier_in_service_area(s, postcode) or (s.get("fit_score", 0) >= 55) for s in pool
    )
    add("service_area", "lead.rule.service_area", area_match)
    add(
        "contact_details",
        "lead.rule.contact_details",
        bool(contact.get("customer_email")) and bool(contact.get("customer_phone")),
    )
