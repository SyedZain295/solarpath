...
def calculate_financing(upfront_cost: float, term_years: int = FINANCING_TERM_YEARS, apr: float = FINANCING_RATE_APR) -> dict:
    if upfront_cost <= 0:
...
def build_financial_model(
    ...
    proj_10 = project_savings_over_years(annual_savings, upfront_cost, 10, battery_kwh=battery_kwh, system_kwp=system_kwp)
    proj_20 = project_savings_over_years(annual_savings, upfront_cost, 20, battery_kwh=battery_kwh, system_kwp=system_kwp)
    ...
    for prod_label, prod_factor in PRODUCTION_SCENARIO.items():
        for price_label, price_esc in ELECTRICITY_ESCALATION.items():
            ...
            scenarios[key] = {
                ...
                "net_10yr": project_savings_over_years(adj_savings, upfront_cost, 10, escalation=price_esc, battery_kwh=battery_kwh, system_kwp=system_kwp)["net_benefit"],
                "net_20yr": project_savings_over_years(adj_savings, upfront_cost, 20, escalation=price_esc, battery_kwh=battery_kwh, system_kwp=system_kwp)["net_benefit"],
            }
